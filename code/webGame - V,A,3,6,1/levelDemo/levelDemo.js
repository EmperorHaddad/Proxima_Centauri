...
*/
class gameMachanics{ /// attempting to put all the code in a class


    tileXYToCanvasXY(tileXY){/// convert the axis, for the game tiles, into the pixel axis for the canvas
        let canvasXY = [,]; /// the variable for the output
        canvasXY[0] = tileXY[0]*64 /// convert the tile  x-axis to the canvas x-axis
        canvasXY[1] = tileXY[1]*48 /// convert the tile  Y-axis to the canvas x-axis
    ...
    clearGameArea(){ /// clears the canvas
        ...
        let ctx1 = this.canvas.context; /// allows edit the canvas
        ctx1.fillStyle = "green"; /// choose the green colour
        ctx1.fillRect(0,0,this.canvas.width,this.canvas.height); /// covers canvas with green
        //console.log(unitRed);
    }

    updateGameArea() {/// calls the 'update' method for each image.
        this.clearGameArea(); /// clear the canvas with green
        this.background.update(); /// add the grid of hexes
        
        for (let i =0;i<this.unitRed.length;i++) { /// add the red units to the canvas
            console.log(i);
            //console.log(unitRed[i]);
            this.unitRed[i].update(); /// add the individual red unit one at a time
        }
        for (let i =0;i<this.unitBlu.length;i++) { /// add the blu units to the canvas
            console.log(i);
            //console.log(unitRed[i]);
            this.unitBlu[i].update(); /// add the individual blu unit one at a time
        }
    }

    addUnit(source,tileX,tileY,isRed){ /// creates a specific unit image (Red or Blu), at a specific place on the canvas  
        
        let canvasXY = this.tileXYToCanvasXY([tileX,tileY]); /// converts grid XY to canvas pixel XY
        let unit = new this.insertImage(source,canvasXY[0],canvasXY[1],64,64,this.canvas); /// unit's image is executed as a class instance
        if (isRed){ /// is unit red?
            this.unitRed[this.unitRedCounter]= unit /// add unit's instance to the list containing red team instances 
            this.unitRedCounter += 1; /// update the array's pointer
        }
        else{
            this.unitBlu[this.unitBluounter]= unit /// add unit's instance to the list containing blu team instances 
            this.unitBluCounter += 1; /// update the array's pointer
        }
    }
    ...
    initialiseUnits(){/// calls the 'addUnit' sub-routine many times to display all units' at initial position.
        ...
        this.background = new this.insertImage("resources/grid.png",0,0,544,400,this.canvas); /// add the image of the grid onto the canvas

        /// add the Red units to the grid:
        this.addUnit("resources/units/playerRed/Tanker.png",0,0,true); /// add the image of a Tanker onto the grid on grid XY [0,0]
        this.addUnit("resources/units/playerRed/Ninjer.png",0,1,true); /// add the image of a Ninjer onto the grid on grid XY [0,1]
        this.addUnit("resources/units/playerRed/Sniper.png",0,2,true); /// add the image of a Sniper onto the grid on grid XY [0,2]
        this.addUnit("resources/units/playerRed/Healer.png",0,3,true); /// add the image of a Healer onto the grid on grid XY [0,3]
        this.addUnit("resources/units/playerRed/Commander.png",0,4,true); /// add the image of a Commander onto the grid on grid XY [0,4]
        this.addUnit("resources/units/playerRed/Sniper.png",0,5,true); /// add the image of a Tanker onto the grid on grid XY [0,5]
        this.addUnit("resources/units/playerRed/Ninjer.png",0,6,true); /// add the image of a Sniper onto the grid on grid XY [0,6]
        this.addUnit("resources/units/playerRed/Tanker.png",0,7,true); /// add the image of a Ninjer onto the grid on grid XY [0,7]
        for (let i = 0; i<8;i++){
            //canvasXY = tileXYToCanvasXY([1,i])
            //unit = new insertImage("resources/units/playerRed/Trooper.png",canvasXY[0],canvasXY[1],64,64);
            this.addUnit("resources/units/playerRed/Trooper.png",1,i,false); /// add the image of a Trooper onto the grid on grid XY [1,i]
        ...
        this.addUnit("resources/units/playerBlu/Tanker.png",7,0,false); /// add the image of a Tanker onto the grid on grid XY [7,0]
        this.addUnit("resources/units/playerBlu/Ninjer.png",7,1,false); /// add the image of a Ninjer onto the grid on grid XY [7,1]
        this.addUnit("resources/units/playerBlu/Sniper.png",7,2,false); /// add the image of a Sniper onto the grid on grid XY [7,2]
        this.addUnit("resources/units/playerBlu/Healer.png",7,4,false); /// add the image of a Healer onto the grid on grid XY [7,4]
        this.addUnit("resources/units/playerBlu/Commander.png",7,3,false); /// add the image of a Commander onto the grid on grid XY [7,3]
        this.addUnit("resources/units/playerBlu/Sniper.png",7,5,false); /// add the image of a Tanker onto the grid on grid XY [7,5]
        this.addUnit("resources/units/playerBlu/Ninjer.png",7,6,false); /// add the image of a Sniper onto the grid on grid XY [7,6]
        this.addUnit("resources/units/playerBlu/Tanker.png",7,7,false); /// add the image of a Ninjer onto the grid on grid XY [7,7]
        for (let i = 0; i<8;i++){
            //canvasXY = tileXYToCanvasXY([6,i])
            //unit = new insertImage("resources/units/playerBlu/Trooper.png",canvasXY[0],canvasXY[1],64,64);
            this.addUnit("resources/units/playerBlu/Trooper.png",6,i,64,64,true); /// add the image of a Trooper onto the grid on grid XY [1,i]
